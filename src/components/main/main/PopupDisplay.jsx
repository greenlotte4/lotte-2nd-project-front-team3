import { fetchPopups, fetchVisiblePopups, hidePopupForUser } from "@/api/popupAPI";
  ...
  const handleHideFor7Days = async () => {
    try {
      if (visiblePopup && user?.id) {
        ...
        setPopups((prev) => prev.filter((popup) => popup.id !== visiblePopup.id));
        handleClose();
      }
    } catch (error) {
  ...
  >
    <div className="bg-white rounded-xl shadow-lg p-8 max-w-xl w-full relative transform transition-transform duration-300">
      ...
      <div className="text-center">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">{visiblePopup.title}</h2>
        <p className="text-gray-600 mb-6 leading-relaxed">{visiblePopup.description}</p>
      </div>
